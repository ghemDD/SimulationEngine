...
//  var log : List[(Int, Double)] = List(),
  ...
  def mycopy(_o: Owner): ProductionLine = {
    val p = this.copy()
    p.o = _o
...
case class HR(private val shared: Simulation,
  ...
  def pay_workers(): Unit = { for (a <- employees) o.transfer_money_to(a, salary); }
  def salary_cost(): Int = salary * employees.length

  def hire(n: Int): Unit = { for (_ <- 1 to n) hire_one(); }

  protected def hire_one(): Unit = {
    if (shared.arbeitsmarkt.nonEmpty)
      employees.push(shared.arbeitsmarkt.pop.asInstanceOf[Person])
  }

  def fire(n: Int): Unit = { for (_ <- 1 to n) fire_one(); }

  protected def fire_one(): Unit = { shared.arbeitsmarkt.push(employees.pop); }
}

class Factory(pls: ProductionLineSpec, shared: Simulation)
    extends SimO(shared) {
  ...
  protected def copy_state_to(_to: Factory,
                              _shared: Simulation,
                              old2new: collection.mutable.Map[SimO, SimO]): Unit = {
  ...
  protected def copy_state_to(_to: Factory): Unit = { assert(false); } // don't call
  ...
  protected def remove_production_line(): Unit = {
    if (pl.nonEmpty) {
  ...
  protected def tactics(): Unit = {
    ...
    def historic_demand: Timeseries[Int] =
      sum_grp[SalesRecord](
