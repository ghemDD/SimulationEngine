...
class MarketMaker(security: Security, shared: Simulation)
  ...
  def mycopy(_shared: Simulation,
             _substitution: collection.mutable.Map[SimO, SimO]): MarketMaker = {
  ...
  /** Not to be used. */
  override def price(dummy: Commodity): Option[Double] = None
  ...
  def action = __do {
    ...
    if (ask0.isDefined && bid0.isDefined) {
      val bid1 = bid0.get
      val ask1 = ask0.get
      ...
      ob.enter_sell_order(
        OB_Line(Some(my_ask_price), shared.timer, vol, Some(1), this))
      ob.enter_buy_order(
        OB_Line(Some(my_bid_price), shared.timer, vol, Some(1), this))
    }
  }
...
class RandomTrader(security: Security, shared: Simulation)
    extends SimO(shared)
    with SimpleSim {
  ...
  def mycopy(
      _shared: Simulation,
      _substitution: collection.mutable.Map[SimO, SimO]): TrendFollower = {
...
class TrendFollower(security: Security, shared: Simulation)
    extends SimO(shared)
    with SimpleSim {

  var l: List[(Double, Double)] = List[(Double, Double)]()
...
class Plotter(security: Security, shared: Simulation)
    extends SimO(shared)
    with SimpleSim {
  import breeze.linalg._
  ...
  /* FIXME: substitute ob. */
  def mycopy(_shared: Simulation,
             _substitution: collection.mutable.Map[SimO, SimO]): Plotter = {
  ...
  def action = __do {
    ...
    if (shared.timer % 5 == 4) {
      def prep(ll: List[Double]) =
        ll.map((x: Double) => if (x != 1.0 / 0) x else 0.0).toArray
...
class Arbitrageur(
  ...
  /* FIXME: substitute ob. */
  def mycopy(_shared: Simulation,
             _substitution: collection.mutable.Map[SimO, SimO]): Arbitrageur = {
  ...
  def action = __do {
    ...
    if ((shared.timer % 5 == 4) && ob_c.ask_price.isDefined) {
      /*
      ...
      val o_val =
        the_option.estimate_price(current_price_c, shared.timer, 100, 100)
...
class OBExample {
  ...
  chicago_builder()
  val ob_c: OrderBook = simu.chicago(Flour)
  val ob_o: OrderBook = simu.chicago(opt)
  ...
  def chicago_builder(): Unit = {
    for (c <- List(Flour, Wheat)) { //Securities.all_commodities) {
      ...
      for (i <- -2 to 2; j <- 1 to 2) {
        ...
        val t = j * 20
        val co = EuropeanCallOption(c, p, t, 0.03)
        simu.chicago += (co -> new OrderBook(co))
        val po = EuropeanPutOption(c, p, t, 0.03)
        simu.chicago += (po -> new OrderBook(po))
      }
    }
  }

  def buy(ob: OrderBook, price: Double, units: Int = 1): Unit = {
    ob.enter_buy_order(OB_Line(Some(price), simu.timer, units, None, b))
  }

  def sell(ob: OrderBook, price: Double, units: Int = 1): Unit = {
    ob.enter_sell_order(OB_Line(Some(price), simu.timer, units, None, s))
  }
